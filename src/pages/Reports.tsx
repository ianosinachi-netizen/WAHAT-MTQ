    ...
    <div className="min-h-screen bg-gray-50 py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
          <div>
            <h1 className="text-4xl font-bold text-gray-900 mb-2">{t('Industrial Reports')}</h1>
            <p className="text-gray-600">{t('Exclusive market insights and compliance documentation for Premium members.')}</p>
          </div>
          <div className="flex items-center space-x-2 bg-teal-50 text-teal-700 px-4 py-2 rounded-full font-bold text-sm">
            <Shield size={18} />
            <span>{t('Premium Access Verified')}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="relative">
            ...
            <select className="w-full pl-12 pr-4 py-4 rounded-2xl border border-gray-200 focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 outline-none transition-all bg-white shadow-sm appearance-none font-semibold text-gray-700">
              ...
              <option>{t('Market Analysis')}</option>
              <option>{t('Compliance')}</option>
              <option>{t('Innovation')}</option>
              <option>{t('Forecasting')}</option>
            </select>
          </div>
        </div>
        ...
        <div className="mt-20 bg-teal-900 rounded-[3rem] p-12 text-center text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h2 className="text-3xl font-bold mb-4">{t('Request Custom Analysis')}</h2>
            <p className="text-teal-100 mb-8 max-w-2xl mx-auto">
              {t('Premium members can request bespoke chemical market reports tailored to their specific industrial requirements.')}
            </p>
